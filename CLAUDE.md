...
**The four NAT rule contexts.** `interface eth N` holds four numbered (`{1-64}`) rule sub-contexts: `port-forward-rule`, `source-nat-rule`, `nat-one-one` and `nat-one-many`. All four are in `blockOpeners`; `natrules.go` models the first three. **Their leaf spellings are irregular and differ between siblings** — `lan-IP <addr>` under port-forward-rule and nat-one-one, but `lan-IP address <subnet>` under source-nat-rule. Quote what the device prints; never reconstruct it from the keyword. Two device-stated rules are easy to get wrong: an absent `overload` leaf means *enabled* (only `disable` is ever printed), and `rule-name` accepts **letters, digits and underscores only** — a hyphen is rejected, so the generic no-whitespace guard used for other free-text leaves is not enough. `nat-one-many` is not implemented: its leaf names are known from the context help but its argument forms were never probed, and this CLI has already proved a leaf's shape cannot be inferred from its name.
...
**A pre-image cannot delete.** `ExtractStanza` snapshots the stanza as it was *before* the change, so replaying it restores an edited leaf but has no way to remove an entity the change created. Any block that **adds** something — a port-forward or source-NAT rule, a filter rule, a geo-ip exception — must set `ConfigBlock.Undo` with the explicit inverse, or its rollback silently reports success and leaves the addition in place (proved live on a port-forward add). The same applies to a block with no `Keys` at all: it snapshots *nothing*, so its rollback sends zero lines. `Undo` also covers flags whose enabled state is the absence of a leaf (inter-vlan-routing, port-scan). `Keys` are the top-level config keys `ExtractStanza` (in `blocktree.go`) uses to cut the rollback pre-image out of the snapshot. `blocktree.go` parses `show config` into nested context blocks using the `blockOpeners` regex list — **add a regex there when a new CLI sub-context is supported**, or its stanza will be flattened into leaves and rollback for that section will be wrong. `blocktree_test.go` round-trip tests against `testdata/show_config_*.txt` guard this.
