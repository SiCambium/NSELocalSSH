...
That second command is not universally available (other firmware, another model, an account without service rights), so `FetchCloudConfig` falls back to deriving the same `CloudConfig` from `show config` via `CloudConfigFromShowConfig` (`cloudconfig_fallback.go`). A device that answers with the CLI's error convention is remembered on the `Client` so the dead round-trip is paid once, not per read; the derived result is cached for `derivedConfigTTL` and dropped by every `RunSequence` (i.e. every write). `CloudConfig.Source` records which path produced a given value, and `/api/config/network` passes it to the frontend as `config_source` so the fields the fallback genuinely cannot derive (VLAN names, the per-VLAN port-scan flag) render as unknown instead of as a confident zero value.
